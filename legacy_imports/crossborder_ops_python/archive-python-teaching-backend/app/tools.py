...
初步判断：{"利润率偏低，建议优先检查广告 ACOS、退款 SKU 和物流成本。" if s.profit_rate < Decimal("18") else "整体利润率可接受，建议继续拆分广告、退款和评论数据寻找增长点。"}"""
...
def refund_risks(db: Session, start_date: str | None = None, end_date: str | None = None, store_name: str | None = None, top_n: int = 5) -> str:
    ...
    for i, row in enumerate(rows, 1):
        lines.append(f"{i}. {row['title']} [{row['skuCode']}]：退款 {row['refunds']} 单，销量 {row['units']} 件，退款率 {row['refundRate']:.2f}%，退款金额 {row['refundAmount']:,.2f}")
...
def high_acos_campaigns(db: Session, start_date: str | None = None, end_date: str | None = None, store_name: str | None = None, threshold_percent: float = 35) -> str:
    ...
    lines = ["高 ACOS 广告活动："]
    for i, row in enumerate(rows, 1):
        lines.append(f"{i}. {row['campaignName']}（{row['channel']}）：花费 {row['spend']:,.2f}，广告销售额 {row['sales']:,.2f}，ACOS {row['acos']:.2f}%，ROAS {row['roas']:.2f}")
